...
namespace ELA_Auth_Service.IntegrationTests.Controllers.V1
{
    ...
    public  class AuthenticationControllerTests : IntegrationTest 
    {
        ...
        [TestMethod]
        public async Task Register_MySqlProblem_ReturnsTokens_Critical_False()
        {
            //Arrange
            var expectedError = "Problem on writing entry in DATA SERVICE DB";
            ...
            var response = await TestClient.PostAsJsonAsync(
                ...
                new UserRegistrationRequest
                {
                    Name = "Sam_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                    Email = $"Sam_{Guid.NewGuid().ToString().Substring(0, 6)}@mail.com",
